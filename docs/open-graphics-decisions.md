...
| z16 | LIGHTS-TOGGLE-RECOMPILE | render | 🐞 **OPEN — maintainer call**, measured 2026-09-18, walk mode, realistic/weak, phone viewport: `setLightsMode('on')` compiles +25 programs on SwiftShader (165→190) and +31 on Metal (207→238) because `FurnitureLights` mounts 19 point/spot lights only while on and three keys programs on the light count; off-toggle compiles 0; steady-state cost is nil (Metal walk p50 16.3 vs 16.5 ms). A ShaderWarmup-style `gl.compile()` pre-warm with the derived fixture set reached Δ0 in ORBIT but left walk unchanged (Δ25/Δ27): `gl.compile()` does not populate the cache for the walk-mode fixture materials even with the correct `numPointLights: 19` in the key. An off-screen `renderer.render()` into a 4×4 target did reach Δ0 but measured 3–4× SwiftShader frame time (p50 278→1013 ms), unexplained; not shipped. Options: keep lights mounted at intensity 0 (per-frame cost while off — the default state), or root-cause the compile-vs-render cache difference. **PARTIAL MITIGATION SHIPPED v0.35.12.3** (`skipShaderLinkChecks`, `docs/audit/perf-trace-2026-09-25.md`): a CDP trace showed the *stutter* is not the compile but the blocking `getProgramInfoLog` + `getProgramParameter(LINK_STATUS)` validation three r184 runs in `WebGLProgram.onFirstUse`; disabling `gl.debug.checkShaderErrors` removes 683 ms of blocking queries and cuts the worst switch-frame gap 717 → 283 ms. The program COUNTS are unchanged, so this item stays OPEN. **⚠️ THAT MITIGATION IS DEFAULT-OFF FOR ONE CYCLE as of `v0.35.17.9`** (owner's call, `docs/audit/code-review-r7-2026-09-25.md`): the perf numbers above stand and the flag is meant to flip back to `true`, but it shipped in the same round as `lighting/boxProjectEnv.ts`, the repo's first hand-written `ShaderChunk` replacement, and a driver-specific rejection of that GLSL would present as black or missing glossy surfaces with a completely clean console. Reporting stays on until `roomProbes` has real-device mileage; `gl.debug.onShaderError` now feeds `scene/shaderLinkError.ts`'s ring buffer so a failure is readable in-app. So z16's stutter is currently UN-mitigated by default. **✅ CLOSED IN WALK MODE by `roomScopedLights` (R7-AE, default on, reversible by flag):** the point lights are a constant pool of 8, mounted dark while the switch is off — the "keep lights mounted at intensity 0" option above, sized by the §9 cost ladder of `docs/research/lights-gpu-bound-2026-09-25.md` rather than at 19. Measured (Apple M4, ANGLE Metal, realistic/capable, DPR 1, living, one boot): legacy switch **+35 programs, 267 ms worst frame** (OS Metal cache warm; R7-AB measured 3.0–7.7 s first-ever) → pool **+0 programs, 16.8 ms worst frame**, on every one of 8 toggles. The price is the lights-OFF state paying 8 dark slots: +2.3 ms (living) / +1.4 ms (corridor), both still 60 Hz. **Still open in orbit**, which lights every fixture (8 → 19 on the switch), and on an orbit-lights-on ↔ walk switch; the maintainer call left is whether orbit should pool too (it would need 12+ slots or merged rooms to keep the dollhouse's per-lamp look) |
