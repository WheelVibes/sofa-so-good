...
| l | WINDOW-LUMINANCE | render + product look | ⏳ **OPEN v0.31.5.236**, figures corrected in `.237`, **re-measured `v0.31.7.247`** — photographs clip 15–39 % of their glazing; the app clips **0.33 %** (≥254) looking level through the north window at 13:00 and **3.61 %** pitched up, so still an order of magnitude short. But the premise needs widening: that opening is covered by **vertical blind slats rendered as shell geometry**, not a bare pane — rays 1 cm apart alternate between hitting the slat plane at z ≈ 1.25 and passing to the sky. So what a walker sees is mostly slats, and making the PANE clip like a photograph would not by itself change the frame; a fix has to decide what the blinds do first. Night (21:00) is already correct and must not regress |
