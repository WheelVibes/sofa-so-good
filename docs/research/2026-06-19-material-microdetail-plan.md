...
| **MAT-004** ✅ | Brushed-metal: `metalBrush.ts` field helper + `getMetalMaterial` (physical+anisotropy), flag-gated. Hairlines via a value-noise lattice (WIDE-U/NARROW-V + drift warp), NOT isotropic fbm (which leaks cross-brush variance). Finish presets stainless/satin/black-steel | M | `procedural/metalBrush.ts` (new), `procedural/metalBrush.test.ts` (new), `furnitureMaterials.ts` (getMetalMaterial + brush singleton), `metalMaterial.test.ts` (new) | **G-furnmat** (`furnitureMaterials.ts`) | RD-401 landed |
| **MAT-004b** ✅ | Wire 8 appliance primitives to `getMetalMaterial` for the `steel` body via the shared `primitives/shared.tsx:applianceBody` helper (steel material on the body mesh `material=` prop; non-steel keeps `applianceFinish` props). `shared.ts`→`shared.tsx` (now exports a JSX component) | S | `furniture/primitives/{Refrigerator,Oven,Stove,RangeHood,Dishwasher,Microwave,WashingMachine,WineCooler}.tsx`, `primitives/shared.tsx`, `applianceBody.test.tsx` (new) | **G-appliances** (own files) | MAT-004 |
