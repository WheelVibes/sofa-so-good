...
| **S4** | ✅ **CLOSED with a stated residual.** The yard opens onto the service light well; the estate is there. | Yard crop (350×360 px), `walk-kitchen-to-yard-door` frame 300: **mean 213.1, ≥240 = 42.2 %, sd 39.5** on HEAD, against **70.9 % / sd 18.6** flag-OFF and 53.6 % / 33.4 at v0.35.6.0. The sd nearly doubling is the view arriving. **Residual: 42 % of that crop is still ≥240** — the wing surfaces run at the blown boost because the adaptive ramp is glazing-driven and the yard has no glazing. Carried forward as N4's sibling. **RESIDUAL ADDRESSED v0.35.12.1 (OPEN-ITEMS-SWEEP):** `estate/apertureCoverage.ts:planOpenWallQuads` now counts the open band above an external half-wall parapet (the yard's `wall-ext-SY-W` + the three AC-ledge walls) as an aperture too, so the re-exposure ramp fires standing in the yard even though it carries no glazing. Flag `apertureOpenWallCoverage` (simple, default true). Verified live (Metal + SwiftShader, yard pose facing `wall-ext-SY-W`): mean luma over the yard/estate crop falls **~15 counts** (201→186 Metal, 200→185 SwiftShader) with the flag on, consistent with the coverage-driven re-exposure; the calibrated `living-far`/`kitchen-east` poses are unit-tested byte-identical (`apertureCoverage.test.ts`). |
...
| **N3** | `walk-orbit-switch-mid-gesture`, all three arms | ✅ **FIXED v0.35.7.2 (MODE-SWITCH-CROSSFADE).** `setCameraMode` no longer raises the boot-brand `LoadingOverlay` — it bumps `cameraSlice.ts`'s `modeTransition`, rendered by `ui/loading/ModeSwitchCrossfade.tsx` as a short unbranded veil (behind `modeSwitchCrossfade`, default on; flag OFF reproduces the exact old splash, re-verified: `/tmp/sweep/mode-switch-crossfade/desktop-metal-ffoff/walk-orbit-switch-mid-gesture/0047.png`). Re-recorded all three arms (`/tmp/sweep/mode-switch-crossfade/`): desktop-metal FLASH 2 / RECOMPILE 5 / STUTTER 2 (both are ordinary content-change flashes on inspection, e.g. `0048.png`→`0049.png` dollhouse→interior — never the splash card); phone-metal DPR_TOGGLE 1 / FLASH 2 / STUTTER 2 / RECOMPILE 2; desktop-swiftshader FLASH 2 / STUTTER 41 (delivery cadence, read structurally only) / RECOMPILE 0. **Second switch of the session costs ~+1 program and one ~133 ms STUTTER on Metal (desktop `255→256` @2690ms, phone `211→212` @2581ms)** — the underlying compile is unchanged (this fix removes the splash, not the compile), but it is now genuinely masked by the veil instead of a 0.4–0.6 s brand card: frame `0119.png` (mid-fade, visibly hazy) → `0120.png` (clear) on desktop-metal shows the veil actually painting. `prefers-reduced-motion` (verified live via CDP `Emulation.setEmulatedMedia`) skips the veil entirely — instant cut, confirmed the veil element never mounts across two consecutive switches. ⚠️ **Residual, left open:** the FIRST switch of a fresh session can cost far more (desktop-metal measured `220→257`, +37 programs) than the second — three's `WebGLBackground` box/plane material (backing `SceneBackdrop.tsx`'s firstPerson-only `scene.background`) is built lazily inside an actual `render()` call, which `ShaderWarmup.tsx`'s existing `gl.compile()`-based pre-warm cannot reach (verified against `three/src/renderers/{webgl/WebGLBackground.js,WebGLRenderer.js}` — `background.render()` is called only from the render path, never from `compile()`). No pre-warm was written for it (would need a hidden forced `gl.render()`, unverified and the same manual-GL-outside-the-loop shape as GPU-STARVE-3/BLOOM-MIP-FLASH); documented in `ShaderWarmup.tsx`'s docstring and `src/scene/CLAUDE.md` as a citation-backed open item, not chased blind. ⚠️ **BACKDROP-WARMUP (2026-09-19): a census (real GPU, fresh session, desktop-metal, `gl.info.programs` `cacheKey`s before/after the first switch, `scripts/dev-probes/census-backdrop.mjs`, deleted after use) reproduced the +37 exactly (218→252, +34 net / 37 added / 3 evicted) and ROOT-CAUSED it — the shape assumed above (a single missing backdrop pre-warm) is only 1 of the 37.** Grouped by shader/material: 27 `physical`/`STANDARD` (furniture+architecture PBR) + 4 `depth` + 4 `basic` + 1 `BackgroundCubeMaterial`. A direct `DirectionalLight` census (`scripts/dev-probes/census-lights.mjs`, deleted after use) found the true cause: **orbit carries 2 directional lights, firstPerson carries 1** — `ORBIT-STUDIO-LOOK`'s orbit-only overhead key light unmounts on the switch, and three bakes `numDirLights`/`numDirLightShadows` into EVERY program's cache key regardless of whether that program's shader reads a light (the LIGHT-COUNT-STABLE mechanism `src/scene/CLAUDE.md` already documents for a fixture-count change, here triggered by the camera mode instead) — so all 37 recompile together, not because 37 different things are walk-only. A control run with NO switch, same timing, held flat (217→217), ruling out asset-streaming coincidence. `ShaderWarmup.tsx` now ships a `warmBackdropProgram` pre-warm for the ONE program this mechanism can name in advance (the background box material, via a forced `gl.render()` of a throwaway scene into a 1×1 offscreen `WebGLRenderTarget` — zero visible frames, restores the render target synchronously, `[probe] backdrop-warmup` reads ~31 ms / 1 program at boot) — **but it does NOT close the gap, verified**: the warm-up runs at boot, still under ORBIT's 2-light census, so it warms a cache key that matches neither orbit (background is walk-only, never used there) nor the walk switch (1-light census) — re-recorded fresh-session desktop-metal post-fix: `walk-orbit-switch-mid-gesture` RECOMPILE `218→249→254→…→253` across 4 events, worst STUTTER **366.7 ms** (worse than this row's `255→256`/133 ms because THIS measurement is a genuine cold first switch, not a second-switch-in-a-warm-session as the number above turns out to be). Rendering the REAL scene instead of a throwaway one (so the warm-up's light census matches the live scene) was tried and REJECTED: it doesn't fix the mismatch (warm-up still runs pre-switch, under orbit's census) and costs 1671.5 ms / 28 programs at boot — compiling every other not-yet-compiled material in the same pass, i.e. moving the stall earlier rather than removing it. **Device-class dependent, confirmed by a natural experiment**: fresh-session phone-metal (weak class) shows only RECOMPILE `206→207→208` (+2) and STUTTER 133.4 ms — matching this row's numbers — because `ORBIT-STUDIO-LOOK`'s gate does not mount the extra key light on `weak`, so phone's orbit and firstPerson censuses already agree and there is no mismatch to warm around. **Recorded as the residual**: closing it structurally would mean `ShaderWarmup` temporarily hiding the studio-key light (by its `STUDIO_KEY_SHADOW_TAG`) and re-running `gl.compile()` under that census — coupling this file to `Lighting.tsx`'s internal light reference for the first time, unverified for cost/correctness, left for a follow-up rather than shipped blind in this pass. | `/tmp/sweep/mode-switch-crossfade/desktop-metal/walk-orbit-switch-mid-gesture/{sheet.png,0048.png,0119.png,0120.png}`, `.../worst/{FLASH-49,FLASH-120,RECOMPILE-52,STUTTER-128}.png`; `/tmp/sweep/mode-switch-crossfade/phone-metal/walk-orbit-switch-mid-gesture/sheet.png`; `/tmp/sweep/mode-switch-crossfade/desktop-swiftshader/walk-orbit-switch-mid-gesture/sheet.png`; flag-off control `/tmp/sweep/mode-switch-crossfade/desktop-metal-ffoff/walk-orbit-switch-mid-gesture/0047.png` | `src/state/slices/cameraSlice.ts` (`setCameraMode` → `modeTransition`), `src/ui/loading/ModeSwitchCrossfade.tsx`, `src/ui/loading/modeCrossfadeTimeline.ts` | ~~**med-high**~~ **FIXED**, residual noted above |
...
| **S4** | ✅ **CLOSED with the stated residual, unchanged.** The yard opens onto the service light well — facing unit, ground below, sky above, planting — on every frame of the walk-out. **Residual stands:** the wing surfaces still run at the blown exterior boost (the adaptive ramp is glazing-driven and the yard has no glazing), visible as a near-white parapet/wing face. Carried as N4's sibling and as `(ag)` in `open-graphics-decisions.md`. **RESIDUAL ADDRESSED v0.35.12.1** — see the twin S4 row above; same fix (`apertureOpenWallCoverage`), same verification. | `walk-kitchen-to-yard-door` 313 frames, **1 event in the whole clip** (one DPR_TOGGLE) | `/tmp/sweep/final2/desktop-metal/walk-kitchen-to-yard-door/sheet.png`, `.../worst/DPR_TOGGLE-271.png` |
...
| **S6** | ✅ **CLOSED (DEGRADE-UNIFIED, v0.35.9.0).** The product question this row left open — desktop halves resolution on a looser one-long-frame/3 s-hold rule while touch already used two-consecutive-frames/1 s — is resolved by extending the touch rule to every pointer type behind `degradeRuleUnified` (`interactiveDegrade.ts:effectiveCoarsePointer`), except the SOFTWARE rasteriser, which keeps the old rule (its certified floor, item (af), depends on it). A/B on `walk-look-drag-while-moving` / `walk-into-wall-slide` / `orbit-slow-rotate` (flag off vs on, same build): DPR_TOGGLE counts and the degraded-sample fraction are unchanged on THIS hardware because none of the three clips produced a genuine >250 ms long frame either way (both arms degrade only via the gesture + 350 ms release debounce) — i.e. **no self-re-arming at rest on either arm**, which is the property this fix targets. The arm/hold semantics themselves are pinned by 6 new unit tests in `interactiveDegrade.test.ts` (fine-pointer desktop still floors at 0.5/DPR-2 floors at 1, unchanged; two consecutive long frames now required and the hold is 1 s, not 3 s; the software-rasteriser path is byte-identical to before). | `events-summary.json` both arms; `clip.json.samples[].dpr`; `interactiveDegrade.test.ts` | `/tmp/photoreal-mobile/sweep/{before,after}-desktop-metal/events-summary.json` |
