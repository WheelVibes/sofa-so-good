...
| GLOSS-BAND-FLAT — the door leaf still reads as corduroy after three rounds of tuning | **Fixed** | Under the existing `doorLeafRealism` flag (simple, default on) — the `door` grain variant is only reachable with the flag on, so no new flag. DOOR-LEAF-REALISM straightened the grain, then two passes halved `reliefScale`/`normalScale` (3 → 0.8 → 0.4, 0.45 → 0.28 → 0.14) on the stated reasoning that "ridge contrast is a RELIEF problem, not an albedo one", and BAND-CONTRAST-VARY added a 0.4–1.0 low-frequency modulation of the ring/pore darkening on top. A real-GPU crop of the bedroom-2 leaf at 0.35 m still showed evenly spaced vertical ribbing over the whole panel, worst in the top third. **Attributed instead of guessed.** Metric: RMS of the leaf crop's per-column mean luminance after subtracting a 41 px moving average — the amplitude of the ribbing — in four horizontal bands, at walk pose (5.2, 4.95) yaw 0, ANGLE Metal / Apple M4, hour 13, realistic; the flat wall beside the door reads **0.048** on the same metric against the leaf's **3.02 / 3.11 / 2.06 / 1.84** (top/upper/mid/low), and the dominant period is 11.75 px = ~19 mm, i.e. exactly the intended 22-ring pitch, so the SPACING was never the defect. Five arms live-patching the drawn material at the pose, one variable each: shipped 3.02/3.11/2.06/1.84 · `normalMap` null **2.99/3.11/2.05/1.84** · `roughnessMap` null 2.42/2.62/3.29/2.80 · albedo `map` null 1.86/1.43/1.14/1.00 · albedo only 2.41/2.62/3.28/2.79. **The relief contributes ~1 % — both relief cuts were aimed at a term that was not doing the damage**, and there are TWO causes: the albedo darkening, and the roughness swing, which is why the top third stayed ribbed after the albedo was cut (that is where the light rakes, so a gloss band shows there first). Two fixes, each swept to its knee: `lateDepth` 0.11 → **0.033** and `poreDepth` 0.09 → **0.027** (whole-leaf RMS at 1.0/0.5/0.3/0.15x: 2.606 / 1.483 / 1.156 / 1.018 — it asymptotes on the roughness floor), and new per-variant `roughLate`/`roughPore` replacing the `late * 0.24 + pore * 0.2` the bake carried inline for every variant, door at **0.072 / 0.06** (per-band at 1.0/0.3/0.1/0: 2.09-1.81-0.39-0.27 / 0.90-0.88-0.53-0.61 / 0.77-0.80-0.85-0.80 / 0.85-0.83-1.02-0.88). Physically: a sawn board scatters more in its open latewood pores; a laminate leaf is a printed sheet under one continuous wear layer. **Shipped 0.90 / 0.88 / 0.53 / 0.61** — down 3.4x at the top and 3.9x in the middle, and now the same size everywhere instead of three times worse where the light rakes. Visually reviewed at three real-GPU poses (bedroom-2 leaf at 0.35 m and 0.15 m, main door from the living room): flush laminate with a faint figure. Furniture is untouched bit-for-bit (same floats, `contrastVary` false multiplies by the exact `1`). `reliefScale` 0.4 / `normalScale` 0.14 are KEPT — a flush laminate leaf is nearly flat — but the docblock and test now say plainly they are not what fixed this. `src/materials/furnitureMaterials.ts`, `woodGrainVariant.test.ts` (14 tests), `src/materials/CLAUDE.md`. |
