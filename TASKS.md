...
- PHOTO-BACKDROP — SHIPPED (see CHANGELOG): `photo` backdrop = equirectangular `scene.background`,
  zero per-frame geometry, rides `backdrops` flag. Follow-ups: swap procedural skyline for a real CC0
  equirectangular photo (sandbox can't fetch); consider making `photo` the default backdrop.
