...
`wall-fabric-denim` (denim_fabric), `wall-leather-white` (leather_white), `wall-metal-plate`
(metal_plate), `floor-wood-natural` (wood_floor), `floor-tile-stone` (floor_tiles_06), and
`floor-concrete` (concrete_floor_02 — overrides the procedural id with the generator kept as the
always-available fallback, the same pattern as the original 12). Albedo+normal+rough at 1K JPG
q90 (AO deliberately skipped — `useTexturedMaterial` doesn't load it); +4.26 MB total; all CC0
with CREDITS.json/md entries (prod-safe). Picker needs no wiring (category filter picks them up).
POM unaffected (textured finishes bypass it by design). New `generatedCatalog.test.ts` integrity
suite (maps exist on disk, credits present, token coverage). Verified on SwiftShader (raster
texturing renders truthfully; the real-GPU path context-lost on these texture-heavy loads this
session): tiling scale/seams/color reviewed across 3 finish pairs + the procedural baseline.
Latent pipeline regression found + tracked (FETCH-MANIFEST-BACKFILL in TASKS.md): re-running
fetch-assets would drop normal/rough channels from 6 older albedo-only manifest entries.
