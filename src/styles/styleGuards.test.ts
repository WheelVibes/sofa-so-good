...
describe('P20 line-height tokens', () => {
  ...
  it('applies --lh-body to multiline descriptions and empty states', () => {
    ...
    expect(flows2).toMatch(/\.empty-sub\b[^}]*line-height:\s*var\(--lh-body\)/s)
    expect(flows2).toMatch(/\.onb-lede\b[^}]*line-height:\s*var\(--lh-body\)/s)
  })
})
