...
// Internal usable rectangle is APARTMENT_EXT_W − 2·ext_wall by APARTMENT_EXT_D − 2·ext_wall.
...
// plus an extension wing south-west of it. Two minor adjustments vs the approved table:
//   • livingDining.depth bumped 7.50 → 7.70 to align its south face with the kitchen's
//     south face (= apartment south wall).
//   • livingDining.extension.depth trimmed 3.70 → 3.50 to balance back to ≈ 90 m² total.
// Net total interior area: 89.65 m² (within the ±0.5 m² tolerance of the 90 m² target).
export const ROOMS: Record<RoomId, RoomDef> = {
  ...
  serviceYard: {
    ...
    width: 1.5,
    depth: 1.4,
    derivation: 'South of bath1, west of kitchen. 1.50 × 1.40 m.',
  },
  kitchen: {
    id: 'kitchen',
    name: 'Kitchen',
  ...
  livingDining: {
    ...
    derivation:
      'L-shape. Main column 3.50 × 7.70 m (east, full apartment height). Extension 4.50 × 3.50 m (south-west wing for foyer + dining). Combined area 42.70 m².',
  },
  acLedge: {
    id: 'acLedge',
    name: 'AC Ledge',
    ...
    depth: 1.5,
    external: true,
    derivation: 'External AC condenser ledge, south of bath1. 2.00 × 1.50 m. Excluded from interior area.',
  },
};
...
export const WALLS: WallSpec[] = [
  ...
  },
  {
    ...
    thickness: 'external',
    cutouts: [
      // L/D bay window on the east side, roughly mid-height of the room.
  ...
  },
  {
    ...
    thickness: 'external',
    cutouts: [
      // Kitchen window above the service yard (offset measured east-to-west since wall runs E→W).
      { kind: 'window', offset: 9.5, width: KITCHEN_WIN_W, sill: WIN_SILL, head: WIN_HEAD, refId: 'win-kitchen' },
    ],
  },
  {
    id: 'wall-ext-W',
    ...
    thickness: 'external',
    cutouts: [
      // Main bedroom secondary window on west side. Offset measured south-to-north.
      { kind: 'window', offset: 6.4, width: BEDROOM_WIN_W, sill: WIN_SILL, head: WIN_HEAD, refId: 'win-mainBedroomW' },
    ],
  },
  ...
  // West wall of kitchen / east wall of serviceYard.
  {
    id: 'wall-int-sy-E',
    start: [1.75, 5.45],
    end: [1.75, APARTMENT_EXT_D],
    thickness: 'internal',
    cutouts: [
      { kind: 'door', offset: 0.3, width: DOOR_W, sill: 0, head: DOOR_HEAD, refId: 'door-serviceYard' },
    ],
  },
  // South wall of serviceYard (separates from… effectively the south external? In our layout
  // serviceYard is depth 1.40, ending at z=6.90. The strip z=[6.90, 7.90] west of x=1.75 is
  // dead space — we leave it as part of serviceYard's footprint with no extra wall).
];
