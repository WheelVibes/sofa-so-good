...
/**
 * PERSISTENCE. `weather` was briefly in the autosave watch-list while `serialize()` did not write
 * it and `shallowEqual` did not compare it — i.e. it could neither trigger a save nor survive one.
 * That is exactly the BUG-001 shape `src/state/CLAUDE.md` warns about, and it passes a naive
 * "is it in PERSISTENT_WATCH_KEYS" check, so these two tests assert the behaviour instead.
 */
describe('weather persistence', () => {
  beforeEach(() => useStore.getState().setWeather('clear'))

  it('round-trips through serialize() → applySerialized', () => {
    useStore.getState().setWeather('overcast')
    const payload = serialize(useStore.getState())
    expect(payload.weather).toBe('overcast')
    const patch = applySerialized(payload, new Set())
    expect(patch.weather).toBe('overcast')
  })

  it('loads a pre-weather save as clear, so an old design renders exactly as it did', () => {
    const payload = serialize(useStore.getState())
    const { weather: _dropped, ...legacy } = payload
    const patch = applySerialized(legacy, new Set())
    expect(patch.weather).toBe('clear')
  })
})
