...
import type { MemberFootprint, SetMemberInstance, SetRecipe } from './ikeaSets'
import {
...
describe('arrangeSet', () => {
  ...
  it('centres the table at the group origin', () => {
    ...
    const placements = arrangeSet(
      instances,
      footprintsFor(instances, { table: TABLE, chair: CHAIR }),
    )
...
describe('arrangeSet — 4 chairs', () => {
  ...
  it('splits 4 chairs 2+2 across opposite edges with no same-edge overlap', () => {
    ...
    const sep = (row: typeof back) => Math.abs(row[0].dx - row[1].dx)
...
const FIXTURE_CATALOG: Record<string, FurnitureDef> = {
  'vihals-gateleg-table': {
    ...
    id: 'vihals-gateleg-table',
    name: 'VIHALS gateleg table',
    category: 'tables',
    paramSchema: [],
  ...
  'vihals-folding-chair': {
    ...
    id: 'vihals-folding-chair',
    name: 'VIHALS folding chair',
    category: 'seating',
    paramSchema: [],
