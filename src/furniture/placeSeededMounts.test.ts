...
describe('SETTLE-ORIGIN: wall-hugging pieces are rescued without losing any', () => {
  ...
  it('CONTROL: pieces that BELONG at the room centre are untouched', () => {
    ...
    //
    // 25 → 24 in `v0.31.7.194`, 24 → 23 in `.196`, back to 24 in `.200`. It moves in BOTH
    // directions for the same reason: enclosing a template's baths for item (f) reshapes the rooms
    // around them, so which pieces happen to land on a room centre changes. `stranded` is
    // unchanged throughout (its assertions still pass), so nothing was displaced either way —
    // that is the guard, and this number is a description rather than a target.
