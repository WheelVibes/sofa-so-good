...
const FIXTURE = {
  ...
  compatibility: {
    accepts_categories: ['Spring mattresses'],
    size: '90x200',
    example_products: [],
  },
  variants: [
...
describe('parseMetadata real-data tolerances', () => {
  ...
  it('accepts glb_materials: [{ textured: true }] only', () => {
    const r = parseMetadata({
      ...base,
      variants: [{ ...base.variants[0], glb_materials: [{ textured: true }] }],
    })
    expect(r.ok).toBe(true)
  })
  it('accepts glb_segments with null mesh/material', () => {
    const r = parseMetadata({
      ...base,
      variants: [{ ...base.variants[0], glb_segments: [{ mesh: null, material: null }] }],
    })
    expect(r.ok).toBe(true)
  })
  it('accepts a variant with no finish field (single-SKU products)', () => {
    const r = parseMetadata({
      ...base,
      variants: [
        { article_number: '00437228', url: 'u', product_title: 'VINNSET Knob', glb: null },
      ],
    })
    expect(r.ok).toBe(true)
  })
  it('parses variant main_image / context_image filenames', () => {
    const r = parseMetadata({
      ...base,
      variants: [
        { ...base.variants[0], main_image: 'white-main.jpg', context_image: 'white-context.jpg' },
      ],
    })
    expect(r.ok).toBe(true)
    if (!r.ok) return
