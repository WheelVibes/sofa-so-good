    ...
    variants: [
      {
        ...
        articleNumber: '1',
        url: 'u',
        assetId: opts.crawled === false ? null : 'x',
        glbMaterials: [],
      },
    ],
...
const bed = group('malm-bed', {
  accepts: ['Spring mattresses', 'Slatted bed bases'],
  size: '90x200',
})
const mattressFit = group('valevag-mattress', {
  hierarchy: ['Mattresses', 'Spring mattresses'],
  size: '90x200',
})
const mattressWrongSize = group('big-mattress', {
  hierarchy: ['Spring mattresses'],
  size: '150x200',
})
...
const uncrawled = group('ghost-mattress', {
  hierarchy: ['Spring mattresses'],
  size: '90x200',
  crawled: false,
})
...
describe('resolveCompatible', () => {
  ...
  it('does not match a different whole-phrase that shares a word (Foam & latex vs Spring)', () => {
    ...
    const springMattress = group('spring-mattress', {
      hierarchy: ['Spring mattresses'],
      size: '90x200',
    })
    const out = resolveCompatible(foamBed, [foamBed, springMattress])
    expect(out['Foam & latex mattresses'].map((g) => g.def.groupKey)).not.toContain(
      'spring-mattress',
    )
    expect(Object.values(out).flat()).toHaveLength(0)
  })
