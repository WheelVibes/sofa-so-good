...
import { combineOnto, resolveStack } from './stacking'
...
describe('combineOnto', () => {
  ...
  it('fails soft (no throw) when a variant is missing', () => {
    ...
    const top = mattressDef()
    const res = combineOnto(
      baseItem,
      base,
      top,
      undefined as unknown as IkeaGltfDef['variants'][number],
