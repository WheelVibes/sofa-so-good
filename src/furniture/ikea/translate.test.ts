...
describe('placementFlags', () => {
  ...
  it('keeps positive front_clearance, omits zero', () => {
    expect(
      placementFlags({ placement: 'floor', semantics: { front_clearance_m: 0.6 } }).frontClearance,
    ).toBe(0.6)
    expect(
      placementFlags({ placement: 'floor', semantics: { front_clearance_m: 0 } }).frontClearance,
    ).toBeUndefined()
  })
})
