  ...
  if (style === 'tub') {
    ...
    return (
      <group>
        ...
        <RoundedBox
          args={[width * 0.92, 0.2, depth * 0.92]}
          radius={0.06}
          smoothness={2}
          castShadow
          position={[0, 0.13, 0]}
        ...
        {/* Seat cushion */}
        <RoundedBox
          args={[width * 0.78, 0.16, depth * 0.7]}
          radius={0.06}
          smoothness={3}
          castShadow
  ...
  return (
    <group>
      ...
      <RoundedBox
        ...
        castShadow
        receiveShadow
        position={[0, baseY + baseH / 2, 0]}
        material={mat}
      />
      {/* Seat cushion */}
      <RoundedBox
        args={[innerW, cushionH, depth - 0.18]}
        radius={0.05}
        smoothness={2}
        castShadow
        position={[0, seatH + cushionH / 2, 0.03]}
        material={mat}
      />
      {/* Back cushion (slightly reclined on the wingback) */}
      <group position={[0, seatH, -depth / 2 + 0.1]} rotation={[wing ? 0.06 : 0, 0, 0]}>
        <RoundedBox
          args={[innerW, backH, 0.18]}
          radius={0.05}
          smoothness={2}
          castShadow
      ...
      {[-1, 1].map((s) => (
        <RoundedBox
          ...
          radius={wing ? 0.06 : 0.05}
          smoothness={2}
          castShadow
          position={[(s * (width - armW)) / 2, armTopY, 0]}
          material={mat}
        />
      ))}
      ...
      {[-1, 1].map((sx) =>
        [-1, 1].map((sz) => (
          ...
          >
            <cylinderGeometry
              args={[0.025, wing ? 0.015 : 0.025, wing ? legH : 0.06, wing ? 10 : 8]}
            />
