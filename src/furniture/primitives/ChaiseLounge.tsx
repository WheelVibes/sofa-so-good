...
export function ChaiseLounge({ props }: { props: ParamProps }) {
  ...
  return (
    <group>
      ...
      <RoundedBox args={[depth, 0.16, len]} radius={0.05} smoothness={3} castShadow receiveShadow position={[0, legH + 0.08, 0]} material={uphol} />
      ...
      <group position={[0, legH + 0.16, -len / 2 + 0.12]} rotation={[-0.32, 0, 0]}>
        <RoundedBox args={[depth, 0.5, 0.16]} radius={0.05} smoothness={3} castShadow position={[0, 0.22, 0]} material={uphol} />
      </group>
      {/* Bolster cushion at the head */}
      <mesh castShadow position={[0, legH + 0.26, -len / 2 + 0.22]} rotation={[0, 0, Math.PI / 2]} material={uphol}>
        <cylinderGeometry args={[0.09, 0.09, depth - 0.06, 16]} />
      </mesh>
      {/* Optional low arm on the back long side (+X) */}
      {arm === 'one' && (
        <RoundedBox args={[0.12, 0.34, len * 0.55]} radius={0.04} smoothness={3} castShadow position={[depth / 2 - 0.04, legH + 0.3, -len * 0.12]} material={uphol} />
      )}
      {/* Legs */}
      {[-1, 1].map((sx) =>
        [-1, 1].map((sz) => (
          <mesh key={`${sx}.${sz}`} castShadow position={[sx * (depth / 2 - 0.08), legH / 2, sz * (len / 2 - 0.1)]} material={legMat}>
