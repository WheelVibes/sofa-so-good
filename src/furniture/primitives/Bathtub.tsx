...
export function Bathtub({ props }: { props: ParamProps }) {
  ...
  const water = { color: '#cfe2e6', roughness: 0.08, metalness: 0, transparent: true, opacity: 0.5 } as const;
  ...
  return (
    <group>
      {/* Outer shell */}
      <RoundedBox args={[width, h - footH, depth]} radius={radius} smoothness={4} castShadow receiveShadow position={[0, footH + (h - footH) / 2, 0]} material={shell} />
      ...
      <RoundedBox args={[width - 0.16, 0.06, depth - 0.16]} radius={radius * 0.6} smoothness={3} position={[0, h - 0.03, 0]} material={shell} />

      {/* Feet (freestanding only) */}
      {style === 'freestanding' &&
        [-1, 1].map((sx) =>
          [-1, 1].map((sz) => (
            <mesh key={`${sx}.${sz}`} castShadow position={[sx * (width / 2 - 0.18), footH / 2, sz * (depth / 2 - 0.14)]}>
