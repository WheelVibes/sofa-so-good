...
export function Vanity({ props }: { props: ParamProps }) {
  ...
  const bulb = { color: '#fff4d8', emissive: '#fff0cc', emissiveIntensity: 0.5, roughness: 0.4 } as const;
        ...
        <group position={[0, tableH + mH / 2 + 0.04, -depth / 2 + 0.05]}>
          ...
          {lights &&
            Array.from({ length: 10 }, (_, i) => {
              ...
              let x = 0;
              let y = 0;
              if (i < per) { x = -mW / 2 + (mW * i) / (per - 1); y = mH / 2 - 0.03; }
              else if (i < per * 2) { x = -mW / 2 + (mW * (i - per)) / (per - 1); y = -mH / 2 + 0.03; }
              else { x = (i % 2 ? 1 : -1) * (mW / 2 - 0.03); y = 0; }
              return (
