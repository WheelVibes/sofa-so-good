...
export function TowelLadder({ props }: { props: ParamProps }) {
  ...
  const FRAME: Record<string, { color: string; rough: number; metal: number }> = {
    chrome: { color: '#cfd3d6', rough: 0.18, metal: 0.95 },
    black: { color: '#2b2d30', rough: 0.4, metal: 0.7 },
    brass: { color: '#b8923f', rough: 0.3, metal: 0.85 },
  }
  const frame = FRAME[finish]
  const mat = frame
    ? getSolidMaterial(frame.color, frame.rough, frame.metal)
