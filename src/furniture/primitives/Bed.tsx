...
export function Bed({ props }: BedProps) {
  ...
  return (
    <group>
      ...
      <RoundedBox
        args={[width + 0.08, 0.1, length * 0.64]}
        radius={0.04}
        smoothness={2}
        castShadow
        receiveShadow
        position={[0, mattressTop + 0.04, length * 0.18]}
        material={duvetMat}
      />
      ...
      {[-1, 1].map((s) => (
        <mesh
          key={`dr${s}`}
          castShadow
          position={[s * (width / 2 + 0.02), mattressTop - 0.04, length * 0.18]}
          material={duvetMat}
        >
      ...
      <RoundedBox
        args={[width - 0.02, 0.05, 0.12]}
        radius={0.025}
        smoothness={2}
        castShadow
        position={[0, mattressTop + 0.06, -length * 0.14]}
        material={duvetMat}
      />
      {/* Pillows */}
