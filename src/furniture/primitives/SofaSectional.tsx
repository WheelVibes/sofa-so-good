  ...
  return (
    <group>
      ...
      <RoundedBox
        ...
        castShadow
        receiveShadow
        position={[0, footH + baseH / 2, mainCz]}
        material={mat}
      />
      {/* Chaise base */}
      <RoundedBox
        args={[chaiseW, baseH, chaiseDepthZ]}
        radius={r}
        smoothness={2}
        castShadow
        receiveShadow
        position={[chaiseCx, footH + baseH / 2, chaiseCz]}
        material={mat}
      />
      ...
      <group position={[0, baseTop, backZ + backThick / 2 + 0.01]} rotation={[recline, 0, 0]}>
        <RoundedBox
          args={[width - armW * 2, backH, backThick]}
          radius={0.05}
          smoothness={2}
          castShadow
      ...
      <RoundedBox
        args={[armW, baseTop + 0.2 - footH, depth]}
        radius={0.06}
        smoothness={2}
        castShadow
        position={[(-chaiseSide * (width - armW)) / 2, footH + (baseTop + 0.2 - footH) / 2, mainCz]}
        material={mat}
      />
      ...
      {Array.from({ length: cushionCount }, (_, i) => {
        ...
        return (
          <RoundedBox
            key={i}
            args={[cw, cushionH, depth - 0.26]}
            radius={0.06}
            smoothness={3}
            castShadow
            position={[x, baseTop + cushionH / 2, mainCz + 0.04]}
            material={mat}
          />
        )
      })}
      ...
      {Array.from({ length: cushionCount }, (_, i) => {
        ...
        return (
          <RoundedBox
            key={`b${i}`}
            args={[cw - 0.02, 0.34, 0.14]}
            radius={0.06}
            smoothness={3}
            castShadow
            position={[x, seatTop + 0.15, backZ + 0.2]}
      ...
      {/* Chaise seat cushion */}
      <RoundedBox
        args={[chaiseW - 0.06, cushionH, chaiseDepthZ - 0.08]}
        radius={0.06}
        smoothness={3}
        castShadow
        position={[chaiseCx, baseTop + cushionH / 2, chaiseCz]}
        material={mat}
      />
      ...
      {[-1, 1].map((s) => (
        <RoundedBox
          ...
          castShadow
          position={[
            chaiseSide * (width / 2 - 0.5) - s * 0.3 * chaiseSide,
            seatTop + 0.16,
            backZ + 0.32,
          ]}
          rotation={[0.3, s * 0.2, s * 0.1]}
