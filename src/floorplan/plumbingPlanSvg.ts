...
export function plumbingSvg(
  ...
  // apart) have their LABELS fanned out with a leader line back to the true
  // circle — see `mepLabelLayout.ts` (H-D1 declutter fix); circle positions
  // are unchanged.
...
/** A single plumbing symbol glyph centred at (cx,cy). When `labelPlacement`
