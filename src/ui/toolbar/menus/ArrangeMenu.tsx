  ...
  return (
    <ToolbarMenu icon="Sets" label="Arrange" active={floorPlanEditing} width="w-64">
      <div className="max-h-[70vh] overflow-y-auto">
        <Action icon="Tidy" label={`Tidy home${chip(shortcutLabel('tidyHome'))}`} sub="Auto-arrange every room" onClick={tidyHome} />
        <Action icon="FloorPlan" label="Floor plan" sub="Edit walls, rooms, doors" active={floorPlanEditing} onClick={toggleFloorPlanEditing} />
        ...
        {recipes.map((r) => (
          <Action key={r.setKey} icon="Sets" label={r.setName} sub="IKEA set" onClick={() => dropIkea(r.setKey)} />
        ))}

        <Header>Presets</Header>
        {LAYOUT_PRESETS.map((p) => (
          <Action key={p.id} icon="Presets" label={p.name} sub={p.description} onClick={() => applyLayoutPreset(p.id)} />
        ))}

        <Header>Style</Header>
        {STYLE_PRESETS.map((p) => (
          <Action key={p.id} icon="Style" label={p.name} onClick={() => applyStyle(p, setFloorFinish, setWallFinish)} />
        ))}
      </div>
    </ToolbarMenu>
  );
...
function Action({ icon, label, sub, active, onClick }: { icon: IconName; label: string; sub?: string; active?: boolean; onClick: () => void }) {
  const Cmp = Icon[icon];
  return (
    <button
      type="button"
