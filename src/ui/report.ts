...
export function buildReportHtml(
  ...
  // Clearance & fit — circulation/walkway-width pinch points. Folds the narrow-
  // walkway check into a single section (only rendered when there's furniture).
  const itemName = (id: string) => {
    const it = items.find((i) => i.id === id)
    return (it && catalog[it.defId]?.name) ?? 'Item'
  }
  const gaps = items.length > 0 ? findNarrowGaps(items, catalog, plan) : []
  const gapLabel = (b: string) => (b.startsWith('wall:') ? 'wall' : esc(itemName(b)))
  const clearanceSection =
    items.length === 0
      ? ''
      : gaps.length === 0
        ? `<div class="room-cost"><h2>Clearance &amp; fit</h2><div class="ok">✓ Walkways look good — every gap between pieces clears the 90 cm ideal.</div></div>`
        : `<div class="room-cost"><h2>Clearance &amp; fit</h2><div class="warn">${gaps.length} narrow walkway${gaps.length === 1 ? '' : 's'} (under the 90 cm HDB ideal):</div><table>${gaps
            .map(
              (g) =>
                `<tr><td class="indent">${esc(itemName(g.a))} ↔ ${gapLabel(g.b)} — ${g.gap.toFixed(2)} m (${g.severity})</td></tr>`,
            )
            .join('')}</table></div>`
...
<style>
  * { box-sizing: border-box; }
  ...
  .room-cost { margin-top: 24px; max-width: 420px; break-inside: avoid; }
  .ok { color: #15803d; }
  .warn { color: #b45309; font-weight: 600; margin-bottom: 4px; }
