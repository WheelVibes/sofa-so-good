...
export function SceneEnvironment() {
  ...
  return (
    <Environment resolution={64} frames={1} background={false}>
      ...
      <Lightformer form="rect" intensity={0.8} color="#ffe6c2" scale={[5, 5, 1]} position={[5, 5, 5]} rotation={[Math.PI / 4, -Math.PI / 4, 0]} />
      {/* Cool counter-fill from the opposite corner so reflections aren't flat. */}
      <Lightformer form="rect" intensity={0.35} color="#c2d4ff" scale={[5, 5, 1]} position={[-5, 4, -5]} rotation={[Math.PI / 4, (3 * Math.PI) / 4, 0]} />
    </Environment>
  );
}
