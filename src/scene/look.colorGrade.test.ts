...
describe('hueSatSaturation', () => {
  it('reproduces the baseline exactly at the default multiplier', () => {
    expect(hueSatSaturation(DEFAULT_SCENE_SATURATION)).toBe(BASE_POST_SATURATION)
  })

  it('keeps the DEFAULT multiplier NEUTRAL', () => {
    // Guards the v0.34.1.19 revert: a 1.65 default measured as an exact match to a photographic
    // reference median and still looked oversaturated. A corpus median is not a perceptual target,
    // and only 20 % of that measured gap was the renderer's in the first place.
    expect(DEFAULT_SCENE_SATURATION).toBe(1)
    expect(hueSatSaturation(DEFAULT_SCENE_SATURATION)).toBe(BASE_POST_SATURATION)
  })
