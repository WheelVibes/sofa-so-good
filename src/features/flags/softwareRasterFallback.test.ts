 ...
 *
 * `default: true` as of `v0.33.2.9`. The wide floor shipped on in `v0.33.2.0`, went
 * off in `v0.33.2.7` on the certified fence table (no reproducible speed win, a
 * flatter frame), and comes back on now that the floor is the NARROW option (3) of
 * `docs/open-graphics-decisions.md` item (af) — post/AO/probe kept, only shadows,
 * DoF, grain and high-DPI dropped, which matches full Realistic to within a point
 * at every luminance percentile while running at flat `performance` speed.
 */
describe('softwareRasterFallback flag', () => {
  ...
  it('is ON in BOTH modes by default', () => {
  ...
  it('can be turned OFF in BOTH modes by a privileged user', () => {
    // The direction that matters while the default is true: an `on` override
